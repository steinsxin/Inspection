...
def check_task(node: ServerSubscriber) -> None:
    ...
    time.sleep(5)
    hw_api = HWAPI()
    piper_voice = PiperVoice(hw_api, "MediumOnnx/zh_CN-huayan-medium.onnx")
    ...
    for i in range(200):
        time.sleep(2)
        piper_voice.play_voice(f"现在是第 {i+1} 次测试")
        print(f"现在是第 {i+1} 次测试")
        ...
        # Reset module status flags
        node.imu = False
        node.is_battery_live = False
        node.rplidar_front = False
        node.rplidar_back = False
